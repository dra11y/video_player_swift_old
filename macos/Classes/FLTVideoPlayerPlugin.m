...
- (FLTFrameUpdater*)initWithRegistry:
...
- (instancetype)initWithAsset:(NSString*)asset
                 frameUpdater:(FLTFrameUpdater*)frameUpdater {
    NSBundle* flutterBundle =
        [NSBundle bundleWithIdentifier:@"io.flutter.flutter.app"];
...
- (void)notifyIfFrameAvailable {
    ...
    if (!_playerItem || _playerItem.status != AVPlayerItemStatusReadyToPlay ||
        ![_videoOutput hasNewPixelBufferForItemTime:outputItemTime]) {
        return;
    } else {
...
static CVReturn OnDisplayLink(CVDisplayLinkRef CV_NONNULL displayLink,
    ...
    __weak FLTVideoPlayer* video_player =
        (__bridge FLTVideoPlayer*)(displayLinkContext);
...
- (void)startDisplayLink {
    ...
    CVDisplayLinkSetOutputCallback(_displayLink, OnDisplayLink,
                                   (__bridge void*)self);
...
- (CGAffineTransform)fixTransform:(AVAssetTrack*)videoTrack {
    ...
    if (transform.tx == 0 && transform.ty == 0) {
        ...
        NSLog(@"TX and TY are 0. Rotation: %ld. Natural width,height: %f, %f",
              (long)rotationDegrees, videoTrack.naturalSize.width,
          ...
          if (tracks.count > 0) {
              ...
              void (^trackCompletionHandler)(void) = ^{
                ...
                if ([videoTrack
                    ...
                    // Video composition can only be used with file-based media
                    // and is not supported for use with media served using HTTP
                    // Live Streaming.
                    AVMutableVideoComposition* videoComposition =
                        [self getVideoCompositionWithTransform:
                                  self->_preferredTransform
                                                     withAsset:asset
    ...
    if (context == timeRangeContext) {
        if (_eventSink != nil) {
            ...
            for (NSValue* rangeValue in [object loadedTimeRanges]) {
                ...
                [values addObject:@[
                    @(start), @(start + FLTCMTimeToMillis(range.duration))
                ]];
            }
...
- (void)sendInitialized {
    if (_eventSink && !_isInitialized) {
        ...
        // The player may be initialized but still needs to determine the
        // duration.
        if ([self duration] == 0) {
            return;
        }
...
- (CVPixelBufferRef)copyPixelBuffer {
    ...
    @synchronized(self) {
        CVPixelBufferLockBaseAddress(_lastValidFrame,
        ...
        CVPixelBufferCreate(kCFAllocatorDefault, bufferWidth, bufferHeight,
                            kCVPixelFormatType_32BGRA,
                            CFBridgingRetain(pixBuffAttributes),
                            &pixelBufferCopy);
        CVPixelBufferLockBaseAddress(pixelBufferCopy, 0);
        uint8_t* copyBaseAddress = CVPixelBufferGetBaseAddress(pixelBufferCopy);
        memcpy(copyBaseAddress, baseAddress, bufferHeight * bytesPerRow);

        CVPixelBufferUnlockBaseAddress(_lastValidFrame,
                                       kCVPixelBufferLock_ReadOnly);
...
@property(readonly, weak, nonatomic)
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
    ...
    // TODO(41471): This should be commented out when 41471's fix lands on
    // stable.
...
- (void)detachFromEngineForRegistrar:
    ...
    // TODO(57151): This should be commented out when 57151's fix lands on
    // stable. This is the correct behavior we never did it in the past and the
    // engine doesn't currently support it.
...
- (FLTTextureMessage*)onPlayerSetup:(FLTVideoPlayer*)player
    ...
    FLTTextureMessage* result =
        [FLTTextureMessage makeWithTextureId:@(textureId)];
...
- (FLTTextureMessage*)create:(FLTCreateMessage*)input
    ...
    FLTFrameUpdater* frameUpdater =
        [[FLTFrameUpdater alloc] initWithRegistry:_registry];
    FLTVideoPlayer* player;
    if (input.asset) {
        NSString* directory;
        if (input.packageName) {
            directory =
                [NSString stringWithFormat:@"flutter_assets/packages/%@/",
                                           input.packageName];
        } else {
        ...
        //      player = [[FLTVideoPlayer alloc] initWithURL:[NSURL
        //      URLWithString:input.uri]
        //                                      frameUpdater:frameUpdater
        //                                       httpHeaders:input.httpHeaders];
        player =
            [[FLTVideoPlayer alloc] initWithURL:[NSURL URLWithString:input.uri]
                                   frameUpdater:frameUpdater];
        return [self onPlayerSetup:player frameUpdater:frameUpdater];
    } else {
...
- (FLTPositionMessage*)position:(FLTTextureMessage*)input
                          error:(FlutterError**)error {
    FLTVideoPlayer* player = _players[input.textureId];
    FLTPositionMessage* result =
        [FLTPositionMessage makeWithTextureId:input.textureId
